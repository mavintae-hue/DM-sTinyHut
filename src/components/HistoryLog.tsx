...
import { Dices, ScrollText, ChevronDown, ChevronUp } from "lucide-react";
...
export default function HistoryLog({ logs }: HistoryLogProps) {
  ...
  return (
    <div 
      className={`flex flex-col ${isCollapsed ? 'h-[64px]' : 'h-[500px] shadow-[0_20px_50px_rgba(0,0,0,0.5)]'} w-full lg:w-[450px] bg-darker/60 backdrop-blur-xl border border-border/50 rounded-2xl overflow-hidden transition-all duration-300 ease-in-out`}
    >
      <div 
        className="p-4 border-b border-border/30 bg-gold/5 flex items-center justify-between cursor-pointer hover:bg-gold/10 transition-colors"
        onClick={() => setIsCollapsed(!isCollapsed)}
      >
        <div className="flex items-center gap-3">
          <ScrollText className="w-5 h-5 text-gold" />
          <h2 className="text-lg font-black tracking-tight" style={{ color: 'var(--theme-font, white)' }}>ROLL LOG</h2>
        </div>
        <div className="flex items-center gap-3">
            <div className="text-[9px] font-bold text-gray-500 uppercase tracking-[0.2em]">Live Sync</div>
