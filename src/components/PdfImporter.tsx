...
import { parseDnDBeyondPdf, ParsedAction } from "@/lib/pdfParser";
...
export default function PdfImporter({ onImport, onClose }: PdfImporterProps) {
  ...
  const handleConfirm = () => {
    const selectedActions = actions.filter((_, i) => selectedIds.includes(i));
    onImport(selectedActions);
  };

  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-[200] flex items-center justify-center p-4">
      <div className="bg-card border border-white/10 rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden animate-in fade-in zoom-in duration-300">
        <div className="flex justify-between items-center p-6 border-b border-white/5 bg-white/5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-xl font-bold text-white tracking-tight">Import Character Sheet</h3>
              <p className="text-xs text-gray-400">Sync actions from D&D Beyond PDF</p>
            </div>
          </div>
          ...
          {actions.length === 0 && !isParsing ? (
            <div className="flex flex-col items-center justify-center py-12 border-2 border-dashed border-white/10 rounded-2xl bg-white/5 hover:bg-white/10 transition-colors group">
              ...
              >
                ...
                <span className="text-white font-bold mb-1">Click to upload or drag & drop</span>
            ...
            <div className="flex flex-col items-center justify-center py-20">
              ...
              <p className="text-white font-medium animate-pulse">Reading Ancient Scrolls...</p>
            </div>
          ) : (
            <div className="space-y-6">
              <div className="flex items-center justify-between p-4 bg-white/5 rounded-2xl border border-white/10 transition-all hover:border-gold/30">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h4 className="text-white font-bold text-lg">{parsedName}</h4>
                    <p className="text-xs text-blue-400 flex items-center gap-1">
                      <Sparkles className="w-3 h-3" /> Found {actions.length} actions in PDF
                    </p>
                  </div>
                </div>
                <button 
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black text-white/40 uppercase tracking-[0.2em] ml-2">Select Actions to Import</label>
                <div className="grid gap-2">
                  {actions.map((action, i) => (
                    ...
                    >
                      ...
                      <span className="text-[10px] text-gray-600 font-mono italic truncate max-w-[150px]">
                        {action.notes}
                      </span>
                    </div>
                  ))}
        ...
        <div className="p-6 bg-white/5 border-t border-white/5 flex justify-end gap-3">
          ...
          <button 
            disabled={selectedIds.length === 0 || actions.length === 0}
          ...
          >
            Import {selectedIds.length} Actions
          </button>
        </div>
      </div>
    </div>
